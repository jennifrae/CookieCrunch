...
class GameScene: SKScene {
    ...
    override init(size: CGSize) {
        ...
        gameLayer.addChild(cropsLayer)
        cropsLayer.addChild(cookiesLayer)
        
        maskLayer.position = layerPosition
        cropsLayer.maskNode = maskLayer
    ...
    func addTiles() {
        ...
        for row in 0...NumRows {
            for column in 0...NumColumns {
                ...
                // The tiles are named from 0 to 15, according to bitmask that is
                // made by combining these four values.
                let value = Int(topLeft) | Int(topRight) << 1 | Int(bottomLeft) << 2 | Int(bottomRight) << 3
                
                // Values 0 - no tiles, 6 and 9 (two opossite tiles) are not drawn
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if success {
            ...
            if let cookie = level.cookieAtColumn(column, row: row) {
                ...
                showSelectionIndicatiorForCookie(cookie)
            }
        }
    }
    ...
    func showSelectionIndicatiorForCookie(cookie: Cookie) {
    ...
    func animateNewCookies(columns: [[Cookie]], completion: () -> ()) {
        ...
        for array in columns {
            ...
            for (idx, cookie) in array.enumerate() {
                ...
                sprite.runAction(
                    SKAction.sequence([
                        SKAction.waitForDuration(delay), SKAction.group([SKAction.fadeInWithDuration(0.05), moveAction, addCookieSound])])
                )
            }
        }
        // 7
