...
class Level {
        ...
        for row in 0..<NumRows {
            for column in 0..<NumColumns {
                if let cookie = cookies[column, row] {
                    ...
                    // Is it possible to swap this cookie with the one on the rihgt?
                    ...
                    // swap witho the one on the above
                    if row < NumRows - 1 {
    ...
    private func detectHorizintalMatches() -> Set<Chain> {
        // 1
        var set = Set<Chain>()
    ...
    func removeMatches() -> Set<Chain> {
        let horizontalChains = detectHorizintalMatches()
    ...
    func calculateScores(chains: Set<Chain>) {
        // 3-chain : 60pt, 4-chain : 120, 5-chain : 180
        for chain in chains {
